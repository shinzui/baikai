...
- Decision: Generalise the bracket/fork entry points (`withTrace`, `withCallLog`, `runRequestWith`) from concrete `IO` to `MonadUnliftIO m =>`. Supersedes the second bullet of the 2026-05-13 `MonadIO`-cascade decision above.
  Rationale: The earlier "`effectful`'s `Eff es` does not satisfy `MonadUnliftIO`" claim was factually wrong. `effectful-core` defines `instance IOE :> es => MonadUnliftIO (Eff es)` in `Effectful.Internal.Monad`, exactly mirroring the `MonadIO` instance. Generalising the bracket signatures is therefore honest, not a false promise. `IO` callers are unaffected (`IO` is the canonical `MonadUnliftIO` instance); a future `baikai-effectful` package no longer needs a separate `withTraceEff` wrapper for these. The implementation uses `Control.Monad.IO.Unlift.withRunInIO` to keep the existing `IO`-bodied implementations untouched. The `TraceSink` fold remains `Fold IO TraceEvent ()` — the prior bullet's reasoning for the *sink contract* still holds even though the *bracket entry point* generalises.
  Date: 2026-05-14
...
- 2026-05-14: Correction to the previous entry. The premise "`Eff es` is not a `MonadUnliftIO`" was wrong — `effectful-core` provides `instance IOE :> es => MonadUnliftIO (Eff es)` in `Effectful.Internal.Monad`. Generalised the bracket/fork entry points `withCallLog`, `withTrace`, and `runRequestWith` from concrete `IO` to `MonadUnliftIO m =>`, implemented via `withRunInIO`. Added `unliftio-core` as a build-dep of the `baikai` library. The `TraceSink` fold stays at `Fold IO TraceEvent ()` (its rationale was independent and still holds). Child plans EP-1, EP-4, and EP-5 carry corresponding Revisions entries. See the 2026-05-14 Decision Log entry for the full reasoning.

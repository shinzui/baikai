...
evidence:
  ...
  - kind: test
    ...
    proves: "The runner against real subprocesses: the prompt is delivered on stdin, stdout and stderr stay separate, a non-zero exit is a successful run carrying that code, a missing working directory and unset declared variables are caught before any spawn, output truncates at the byte limit, inherit mode captures nothing, and a timeout terminates the process group including grandchildren."
  - kind: test
...
grants a grace period, then terminates — so the agent's own children go with it.

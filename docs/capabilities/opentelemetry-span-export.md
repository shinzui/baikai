...
- `gen_ai.response.model` is set **only when the provider actually reported a
  model**. It is deliberately not backfilled from the request, so its absence on
  a span is information rather than a gap.

...
- __Two more pinned tests than the plan named had to be re-based on a reasoning
  ...
  (2026-08-27, M3)
- __The formatter exclusion that protects `Generated.hs` is not reliably present,
  so the generator now emits canonical layout instead of relying on it.__ The
  generator's first compat rendering laid the record update out as
  `compat = CompatAnthropicMessages` with the record on following lines; the
  `treefmt` pre-commit hook rewrote that to `compat =` with the constructor on its
  own line and the braces two columns deeper, and the commit failed — which
  `CatalogSpec`'s byte-identity check would then have failed too.
  `nix/pre-commit.nix` has excluded `Generated.hs` at the hook level since
  `6af3032` (2026-06-21) for exactly this reason, but `.pre-commit-config.yaml` is
  gitignored and generated by `git-hooks.nix` on dev-shell entry; the copy in this
  checkout predates that commit and carries `"exclude": "^$"`. So the exclusion is
  real and correct, and simply was not active here. `renderCompat` now returns
  source lines in the layout the formatter produces, which makes the round trip
  hold whether or not the exclusion is in force — a better guarantee than the
  exclusion alone. Any future change to the rendered layout must be checked
  against `nix fmt`, not only against the compiler. (2026-08-27, M1)
...
- Decision: No second ADR beyond `0009`; the "generated output is emitted in the
  formatter's canonical layout" rule stays in this plan and in `renderCompat`'s
  Haddock.
  Rationale: the repository has one generator, and `nix/pre-commit.nix` already
  carries the constraint and its explanation at the place that enforces it. A
  durable record would restate a comment that already exists next to the code it
  governs. `docs/capabilities/generated-model-catalog.md`'s claim that the file is
  excluded from the formatter stays true — it is, when the hook config is current.
  Date: 2026-08-27
...
Complete, 2026-08-27, four milestones in four commits: `29daac0` (catalog record),
`1b5ed8a` (Anthropic adapter), `54b992b` (OpenAI reasoning gate), `5fc558b` (pins,
smoke cases and documentation).

What holds now that did not. A thinking request on `anthropic_claude_sonnet_5` sends
`"thinking":{"type":"adaptive"}` and no `budget_tokens`, because the shape is a field
of the model's generated catalog record rather than a guess from its id; the prefix
table that did not know the id is deprecated and consulted by nothing. `temperature`
and `top_p` are omitted for a generation that rejects them and the omission is recorded
as `sampling_dropped_unsupported_model`; `seed` and the two penalties, which the
Messages API has no field for on any generation, are recorded as
`sampling_dropped_unsupported_api`; neither refuses a strict call, because
`weakensThinking` says they do not weaken thinking. A model with an unknown output cap
sends 1024 rather than `max_tokens: 0`, and no longer loses its whole thinking plan to
a ceiling of zero. Replay never sends an empty text block or an empty content array.
Tool-call ids that used to collide carry a hash suffix, and a residual collision in one
turn is refused rather than misrouted. On the OpenAI-compatible side, a level on a
model whose catalog entry says `reasoning = False` sends no reasoning control on any
host and records `thinking_dropped_unsupported_model`, so
`docs/user/model-call-evidence.md`'s baikai-wide promise is now true on both API
adapters.

Findings closed: REV-2 C.1 through C.5, C.7 and C.8's adapter half (C.6 is a documented
limitation with a pin, as the plan decided); Theme I items 3 and 4; the REV-1 residual
on the Claude `max_tokens: 0` case.

The gate. `cabal test all` under the keyless environment passes all eight suites — 575
`baikai`, 260 `baikai-claude`, 170 `baikai-openai`, 87 `baikai-agent`, plus
`baikai-kit`, `baikai-trace-otel`, `baikai-effectful` and `baikai-smoke` — and
`baikai-smoke` prints the two `claude-sonnet-5` skip lines. `nix fmt` leaves the tree
unchanged and `okf validate docs/capabilities` reports `OK: 22 concepts`.

Not done, deliberately. No keyed run happened: no Anthropic key is assumed in this
environment, so the facts table is written from the API reference REV-2 consulted and
the two `ThinkingSmoke` cases are what a later keyed run must show. The riskiest row is
`claude-sonnet-4-6`, which this plan flipped from budget to adaptive on the strength of
the reference; if a keyed run finds otherwise, the fix is one `anthropicInclude` entry,
a regeneration, and a Decision Log note. The single cache-write rate (C.6) is a
documented limitation rather than a fix, for the reason in the Decision Log: the split
needs a value the SDK does not carry, a field EP-8 owns, and a rate models.dev does not
publish.

Breaking changes recorded under `[Unreleased]` for EP-10 to version: `baikai`'s
`AnthropicMessagesCompat` gains a field (constructor unexported, so record-update
callers are unaffected) and `evidenceSchemaVersion` moves to `1.1`;
`baikai-openai`'s four `Shape` functions take a `Bool` after the compat record.
`defaultAnthropicThinkingStyle` is deprecated and belongs on EP-10's removal list.

Handoffs. EP-8 owns `immediateError`'s evidence content (C.8's other half) and must
keep `describeThinkingFor` there, must record the two constructors and `weakensThinking`
in its own Decision Log, and must set the schema version to `2.0` without re-bumping
past this plan's `1.1`. EP-10 keeps `AnthropicMessagesCompat`'s constructor unexported,
removes `defaultAnthropicThinkingStyle`, and may relocate the new `Request.hs` seams
(`planRequest`, `SamplingPlan`, `uncappedMaxTokensFloor`, `normalizeToolCallId`) behind
`.Internal`. EP-11 reconciles the guide and capability text this plan wrote.

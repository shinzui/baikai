...
- 2026-05-14: Correction. The "`effectful` does not satisfy `MonadUnliftIO`" claim above was wrong — `effectful-core` provides `instance IOE :> es => MonadUnliftIO (Eff es)`. Generalised `withTrace` and `runRequestWith` from concrete `IO` to `MonadUnliftIO m =>`, implemented via `Control.Monad.IO.Unlift.withRunInIO` so the existing IO bodies are reused unchanged. `fileSink` and the `TraceSink` shape (`Fold IO TraceEvent ()`) stay as before — those decisions are about the sink contract that runs inside the forked worker, which is independent of the entry-point polymorphism. Driver: the 2026-05-14 correction recorded in the masterplan's Decision Log.
